...
const ExperienceCard = ({ exp }: { exp: any }) => {
  return (
    ...
    >
      <div className=''>
        <h3 className='text-light text-2xl md:text-xl sm:text-lg font-bold'>
          {exp.role}
        </h3>
        <p
      ...
      <ul className='mt-5 list-disc ml-5 space-y-2'>
        {exp.points?.map((point: string, index: number) => (
          <li
            key={`exp-point-${index}`}
            className='text-sm md:text-xs text-light pl-1 tracking-wider'
          >
            {point}
          </li>
        ))}
      ...
      <p className='text-gray-400 font-medium' style={{ fontSize: '14px' }}>
        {exp.startingDate} - {exp.currentlyWorking ? 'Present' : exp.endingDate}
      </p>
    </VerticalTimelineElement>
  );
};
  ...
  return (
    <Layout classStyles='!pt-16 px-20'>
      <AnimatedText
        ...
        classStyles='xl:!text-7xl lg:!text-6xl md:!text-5xl'
      />
