...
const CustomLink = ({
  ...
  return (
    <Link href={href} className={`${classStyles} relative group`}>
      {title}
      <span
        className={`h-[2px] inline-block group-hover:w-full transition-[width] ease duration-300 bg-dark dark:bg-light absolute left-0 -bottom-0.5 ${
...
const CustomMobileLink = ({
  ...
  return (
    <button
      onClick={handleClick}
      className={`${classStyles} relative my-2 text-light dark:text-dark group`}
    >
      {title}
      <span
        className={`h-[2px] inline-block group-hover:w-full transition-[width] ease duration-300 bg-light dark:bg-dark absolute left-0 -bottom-0.5 ${
          router.asPath === href ? 'w-full' : 'w-0'
        }`}
      >
        &nbsp;
...
const Navbar = () => {
  ...
  return (
    <header className='w-full px-24 xl:px-10 lg:px-16 md:px-12 sm:px-8 py-8 font-medium flex justify-between items-center dark:text-light relative z-10'>
      ...
      >
        <span
          className={`bg-dark dark:bg-light block transition-all duration-300 ease-out h-1 w-8 rounded-sm ${
            isOpen ? 'rotate-45 translate-y-2' : '-translate-y-0.5'
          }`}
        ></span>
        <span
          className={`bg-dark dark:bg-light block transition-all duration-300 ease-out h-1 w-8 rounded-sm my-1 ${
